...
const ModernDataVisualization = ({ data }: { data: Data }) => {
	...
	const cardData = [
		{ title: "Prime Score", value: data.data.scores.prime_score, icon: Award },
		{
			...
			value: data.data.scores.community_score,
			icon: Users,
		},
		{
			title: "Initialization Score",
			value: data.data.scores.initialization_score,
			icon: Zap,
		},
		{
			title: "Prime Amount Cached",
			value: data.data.prime_amount_cached / 1e18,
			icon: Award,
		},
		{
			title: "Prime Held Duration (days)",
			value: Math.round(data.data.prime_held_duration / 86400),
			icon: Clock,
		},
		{ title: "Users Referred", value: data.data.users_referred, icon: Users },
		{
			title: "Inactive Referrals",
			value: data.data.extra.inactive_referrals,
			icon: Users,
		},
		{
			title: "Echelon Governance Participation",
			value: data.data.echelon_governance_participation,
			icon: Vote,
		},
	];
			...
			<Card className="bg-gradient-to-r from-purple-400 to-pink-500 text-white">
				...
				<CardContent>
					...
					<p>
						...
						{data.data.held_prime_before_unlock ? "Yes" : "No"}
					</p>
					<p>
						Participated in Prime Unlock Vote:{" "}
						{data.data.participated_in_prime_unlock_vote ? "Yes" : "No"}
					</p>
				</CardContent>
			</Card>
